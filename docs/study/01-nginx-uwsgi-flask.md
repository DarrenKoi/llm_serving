...
### 7.1 TLS 는 어떻게 생겼고, 어떤 특징을 가지는가

#### 왜 생겼나

초기의 HTTP는 **평문 통신**이었다. 같은 네트워크 구간에서 패킷을 볼 수 있는
사람은 비밀번호와 요청 내용을 읽거나, 응답을 바꾸거나, 가짜 서버인 척할 수
있었다. 웹에서 로그인과 결제를 하려면 다음 세 가지가 필요했다.

1. **기밀성**: 중간에서 내용을 읽지 못하게 한다.
2. **무결성**: 전송 중 내용이 바뀌면 알아낸다.
3. **인증**: 접속한 서버가 정말 그 도메인의 서버인지 확인한다.

Netscape가 이를 위해 1990년대에 **SSL(Secure Sockets Layer)**을 만들었다.
SSL 2.0과 3.0을 거친 뒤, IETF가 SSL 3.0을 바탕으로 표준화한 것이 1999년의
**TLS 1.0(Transport Layer Security)**이다. 이후 TLS 1.2와 1.3으로 발전했다.
오늘날 "SSL 인증서"라는 표현은 관습적으로 남아 있지만 실제 통신에는 TLS를 쓴다.
오래된 SSL 프로토콜 자체는 안전하지 않아 사용하지 않는다.

```text
HTTP                         HTTPS
요청·응답을 평문으로 전송      HTTP를 TLS라는 보안 통로 안에서 전송

HTTP + TLS = HTTPS
```

TLS는 HTTP 전용 암호화가 아니다. 애플리케이션 데이터 아래에 보안 통로를 만들기
때문에 이메일, 데이터베이스 연결 등 다른 프로토콜도 TLS를 사용할 수 있다.

#### 연결할 때 무슨 일이 일어나나

브라우저가 `https://example.com`에 접속하면 HTTP 요청을 보내기 전에 TLS
**핸드셰이크(handshake)**를 한다. TLS 1.3을 단순화하면 다음 순서다.

```text
클라이언트                              서버
   │ ── 지원 버전·암호 방식·키 재료 ──▶ │  ClientHello
   │ ◀── 선택 결과·인증서·키 재료 ───── │  ServerHello 등
   │                                     │
   │  인증서의 도메인·유효기간·CA 서명을 확인
   │  양쪽이 같은 세션 키를 각각 계산
   │                                     │
   │ ◀════ 대칭키로 암호화된 HTTP ═════▶ │
```

여기에는 서로 다른 암호 기술이 역할을 나눠 쓴다.

- **인증서와 전자서명**: 서버가 해당 도메인의 개인키를 가졌음을 증명한다.
- **키 교환**: 도청자에게 노출하지 않고 이번 연결의 세션 키를 합의한다.
- **대칭키 암호화**: 합의한 세션 키로 실제 요청과 응답을 빠르게 암호화하고
  변조 여부도 함께 검증한다.

공개키 암호로 모든 HTTP 데이터를 암호화하는 것이 아니다. 공개키 계열 연산은
상대 인증과 키 합의에 사용하고, 대용량 데이터에는 훨씬 빠른 대칭키를 사용한다.

#### 인증서는 무엇을 증명하나

서버는 보통 **인증서, 개인키, 인증서 체인**을 가진다.

- 인증서: `example.com` 같은 이름과 서버의 공개키를 묶은 문서
- 개인키: 서버만 보관하는 비밀. 외부로 유출되면 안 된다.
- CA(Certificate Authority): 그 묶음이 유효하다고 서명해 주는 신뢰 기관

브라우저는 운영체제나 브라우저에 미리 들어 있는 신뢰 CA 목록에서 시작해 서버가
보낸 인증서 체인의 서명을 확인한다. 서명이 유효해도 접속한 도메인과 인증서의
이름이 다르거나 유효기간이 지났으면 경고한다.

기본 TLS는 **서버가 누구인지**를 인증한다. 서버는 사용자를 로그인·API 토큰 같은
애플리케이션 방식으로 확인한다. 클라이언트도 인증서를 제시하게 만들 수 있는데,
이를 **mTLS(mutual TLS)**라고 한다.

#### TLS 1.3의 중요한 특징

- **전송 내용의 기밀성과 무결성**을 함께 제공한다.
- 일반적인 인증서 기반 연결은 ECDHE 임시 키를 사용해, 나중에 서버 개인키가
  유출되어도 과거에 녹화된 통신을 쉽게 복호화할 수 없게 한다. 이를
  **전방향 안전성**이라 한다.
- 이전 연결 정보를 이용하는 세션 재개로 다음 연결의 핸드셰이크 비용을 줄일 수 있다.
- 오래되거나 취약한 암호 방식과 불필요한 선택지를 줄여 TLS 1.2보다 구성이 단순하다.

다만 TLS가 모든 보안을 해결하지는 않는다.

- 암호화되기 전 클라이언트와 복호화된 뒤 서버의 데이터는 보호하지 못한다.
- 인증서는 **도메인과 공개키의 관계**를 확인할 뿐, 그 사이트가 선량하다는 보증은 아니다.
- 통신 내용은 숨기지만 IP 주소, 통신 시점, 데이터 크기 같은 메타데이터는 남는다.

#### nginx의 "TLS 종료"는 무슨 뜻인가

이 문서의 구조에서는 TLS 연결의 상대가 Flask가 아니라 **nginx**다.

```text
클라이언트 ═══ HTTPS/TLS ═══▶ nginx ─── HTTP/uwsgi ───▶ uWSGI ─▶ Flask
                               ▲
                         여기서 복호화한다
```

nginx가 인증서와 개인키를 들고 핸드셰이크와 암복호화를 처리한다. 복호화된 HTTP
요청을 내부의 uWSGI나 Gunicorn으로 넘기는 지점에서 외부 TLS 연결이 끝나므로
**TLS termination(종료)**이라고 부른다. 덕분에 인증서 갱신과 암호 설정을 nginx
한 곳에서 관리할 수 있고, 애플리케이션은 평범한 HTTP 요청만 처리하면 된다.

그 대신 `nginx → 애플리케이션 서버` 구간이 평문이면 그 내부 네트워크를 신뢰한다는
전제가 생긴다. 장비나 네트워크를 넘어가는 구간까지 보호해야 한다면 내부 구간에도
TLS를 적용하거나 mTLS로 양쪽을 인증해야 한다.
